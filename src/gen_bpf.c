					      ...
					      const struct bpf_jump *nxt_jump)
{
	...
	for (iter = 0; iter < blk->blk_cnt; iter++) {
		...
		case TGT_PTR_DB:
			...
			b_new = _gen_bpf_chain(state, sys, node, nxt_jump);
			if (b_new == NULL)
				return NULL;
			i_iter->jt = _BPF_JMP_HSH(b_new->hash);
			break;
 ...
 * @param first this is the first syscall for the architecture
 *
					...
					uint64_t nxt_hash, int first)
{
	...
	blk_c = _gen_bpf_chain(state, sys, sys->chains, &def_jump);
	if (blk_c == NULL)
		return NULL;

	/* load the syscall into the accumulator (if needed) */
	if (first) {
