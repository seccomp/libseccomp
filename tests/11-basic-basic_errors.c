...
int main(int argc, char *argv[])
{
	...
	/* seccomp_export_bpf_buf errors */
	char buf[1024];
	size_t buf_len = sizeof(buf);
	rc = seccomp_export_bpf_buf(ctx, buf, &buf_len);
	if (rc != -EINVAL)
		return -1;
	...
	if (ctx == NULL)
		return -1;
	rc = seccomp_export_bpf_buf(ctx, buf, NULL);
	if (rc != -EINVAL)
		return -1;
	rc = seccomp_export_bpf_buf(ctx, NULL, NULL);
	if (rc != -EINVAL)
		return -1;

	rc = seccomp_export_bpf_buf(ctx, NULL, &buf_len);
	if (rc != 0)
		return -1;
	rc = seccomp_export_bpf_buf(ctx, buf, &buf_len);
	if (rc != 0)
